...
class QAGenerator:
    ...
    def generate_questions_from_inputs(self, qg_inputs: List) -> List[str]:
        ...
        for qg_input in qg_inputs:
            ...
            gen_qns_context.append(qg_input)
            generated_questions.append(question)

        eval_qn_accuracy(generated_questions, gen_qns_context)
...
class BoolQAGenerator:
    ...
    def get_bool_qnsans(self, document, no_of_qns=5):
        ...
        output = self.beam_search_decoding(
            input_ids, attention_masks, no_of_qns)
        bool_qns_ans = list()
...
def eval_qn_accuracy(qa_list, qg_inputs):
    from transformers import pipeline
    question_answerer = pipeline('question-answering')
    contexts_list = []
    question_score = []
    for st in qg_inputs:
        match = re.search('<context>(.*)\.', st)
        if match:
            contexts_list.append(match.group(1).strip())
    for q, c in zip(qa_list, contexts_list):
        final_score_list = question_answerer(question=q, context=c)
    ...
    if (question_score):
        overall_accuracy = float(sum(question_score)/len(question_score))
...
def print_qa(qa_list, isSingle=False):
    ...
    if qa_list and isinstance(qa_list[0]['answer'], list):
        if isSingle == False:
            return mcq_qns_ans
        return wh_qns_ans
    return subj_qns_ans
